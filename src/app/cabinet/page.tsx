...
export default function CabinetPage() {
  ...
  const load = useCallback(async () => {
    ...
    setWorks(w.works ?? []);
    // стаж — приватный, отдаём в /api/profile
    const now = Date.now();
    setMemberSince("");
    void now;
  }, [router]);
  ...
  async function saveProfile() {
    ...
    const res = await fetch("/api/profile", {
      ...
      body: JSON.stringify({ username, displayName, avatarEmoji, avatarUrl, bio, contacts }),
    });
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-2">
        ...
        <div className="card p-6">
          ...
          <div className="mt-5 space-y-4">
            ...
            <label className="block">
              <span className="mb-1.5 block text-sm text-zinc-400">Юзернейм (по нему вас найдут: /u/username)</span>
