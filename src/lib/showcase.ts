...
/**
 * Пулы специализаций по ролям — чем шире пул, тем «живее» сообщество:
 * у соседних демо-профилей почти не бывает одинаковых специфик.
 * Пары «специализация + предметная область» комбинируются при генерации.
 */
const SPECIALIZATION_TITLES: Record<string, string[]> = {
  developer: ["Fullstack-разработчик", "Frontend-разработчик", "Backend-разработчик", "Python-разработчик", "Mobile-разработчик", "Go-разработчик", "Ruby-разработчик", "PHP-разработчик", "Java-разработчик", "C#/.NET-разработчик"],
  osint: ["OSINT-аналитик", "Аналитик открытых источников", "Специалист по цифровой форензике", "Исследователь инфраструктуры"],
  "osint-author": ["Автор OSINT-кейсов", "Кейсист", "Исследователь даркнета", "Автор расследований"],
  "osint-revealer": ["Раскрыватель кейсов", "Специалист по деанону инфраструктуры", "Разработчик связных графов"],
  designer: ["UI/UX-дизайнер", "Иллюстратор", "Бренд-дизайнер", "Product-дизайнер", "Моушн-дизайнер", "3D-художник"],
  tester: ["QA-инженер", "Тестировщик автоматизации", "QA-аналитик", "Нагрузочный тестировщик"],
  devops: ["DevOps-инженер", "SRE", "Инженер инфраструктуры", "Специалист по CI/CD"],
  analyst: ["Продуктовый аналитик", "Data-аналитик", "BI-аналитик", "Системный аналитик"],
  marketer: ["Performance-маркетолог", "SMM-стратег", "SEO-специалист", "Контент-маркетолог"],
  writer: ["Технический копирайтер", "Автор кейсов и гайдов", "Редактор документации", "UX-писатель"],
  gamedev: ["Инди-геймдев", "Геймдизайнер", "Unity-разработчик", "Графический программист"],
  other: ["Мультидисциплинарный специалист", "Продюсер цифровых проектов", "Исследователь и спикер"],
};

/** Предметные области — добавляются к специализации для разнообразия. */
const SPECIALIZATION_DOMAINS = ["в финтехе", "в e-commerce", "в геймдеве", "в медиа", "в образовании", "в блокчейне", "в кибербезопасности", "в здравоохранении", "в логистике", "в стриминге", "в SaaS", "в блогинге"];

/** Полный пул специализаций: заголовок (± домен) на каждую роль. */
const SPECIALIZATIONS: Record<string, string[]> = Object.fromEntries(
  Object.entries(SPECIALIZATION_TITLES).map(([role, titles]) => [
    role,
    [...titles, ...titles.slice(0, Math.ceil(titles.length / 2)).map((t) => `${t} ${pick2(SPECIALIZATION_DOMAINS)}`)],
  ])
);

/** Независимый выбор из массива (используется до инициализации основного ГПСЧ). */
...
export function generateDemoCommunity(count: number): { users: GeneratedDemoUser[]; works: DemoWork[] } {
  ...
  for (let i = 0; i < total; i++) {
    ...
    // Публичный ID — случайные 8 цифр, как у настоящих пользователей
    let id = "";
    for (let attempt = 0; attempt < 10; attempt++) {
      const candidate = String(Math.floor(10_000_000 + rnd() * 90_000_000));
